...
**Anthropic / OpenAI**：两个服务分别使用独立策略组，且其 `DOMAIN-SUFFIX` 规则位于通用「AI服务」规则之前，因此可单独选择出口。域名由构建脚本从 [v2fly/domain-list-community](https://github.com/v2fly/domain-list-community) submodule 的 `anthropic` 与 `openai` 数据自动提取；无法转换成 `DOMAIN-SUFFIX` 的 `regexp:` 条目不会加入。
